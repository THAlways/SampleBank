...
| Area | Description |
|------|-------------|
| **Top bar** | Shows the app name, version `v.O1.1.2`, current user and a link to Bossard’s corporate website. |
| **Toolbar** | Contains buttons to add items, export/save data, import data, view audit logs, create backups and log into the admin panel. |
...
Click the item’s photo or the blank area to open a detail dialog.  From there you can edit notes, find equivalent items (filter by similar properties) or edit the item.

### BN Links

In version O1.1.2 the **BN** number links directly to Bossard’s e‑shop using the new permalink format: `https://www.bossard.com/global-en/eshop/search/p/<BN>/`.  For example, BN 2098 opens `https://www.bossard.com/global-en/eshop/search/p/2098/`.  Internet access is required for these links.  Article numbers are no longer linked.
...
Every action (add, edit, delete, note change, stock change) is recorded.  Click **Log** to open the audit log dialog.  Use **Export .txt** to download the log.
